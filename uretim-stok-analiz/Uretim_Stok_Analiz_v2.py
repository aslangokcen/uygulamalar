...
# -*- coding: utf-8 -*-
"""
Üretim ve Stok Analiz Paneli  v2
...
# ── PDF: Sevk (Günlük Sevk Raporu) ───────────────────────────────────────────
# Tablo sütunları: [0]=Fabrika [3]=İçPiyasa M2 [8]=İhracat M2 [13]=Toplam M2
...
def extract_sevk_pdf(path: Path) -> Optional[dict]:
    try:
        with pdfplumber.open(str(path)) as pdf:
            ...
            if not tables:
                return None
            for row in tables[0][3:]:   # ilk 3 satır başlık
                if not row or not row[0]:
                    continue
                fab = row[0].strip()
                if fab == "Toplam":
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # Veri depoları
        self._uretim_data: dict[date, dict] = {}   # {date → {fab → {kal1,…}}}
        self._sevk_data:   dict[date, dict] = {}   # {date → {fab → {ic_piyasa,…}}}
        ...
        # Figürler — build_ui'de oluşturulur
        self._figs  = [None] * 6
        self._axes  = [None] * 6
        self._ax3r  = None
    ...
    def _build_tab4(self, p):
        ...
        ttk.Checkbutton(ctrl, text="Günlük Genel Toplam",
                        ...
                        command=self._draw_chart4).pack(side="left", padx=4)
        self._make_chart(p, 3)

    def _build_tab5(self, p):
        # İki seçim satırı: hangi üretim metriki, hangi sevk metriki
    ...
    def _draw_chart1(self):
        ...
        for fab in self._active_fabs():
            for k, (lbl, ls, mk) in C1_METRICS.items():
                ...
                ax.plot(x, self._u_series(k, fab, dates), color=FAB_COLORS[fab],
    ...
    def _draw_chart2(self):
        ...
        for fab in self._active_fabs():
            for k, (lbl, ls, mk) in C2_RATIOS.items():
                ...
                num = k.replace("_pct", "")
                ax.plot(x, self._ratio(num, fab, dates), color=FAB_COLORS[fab],
                        ls=ls, marker=mk, markersize=4, lw=1.6,
                        label=f"{fab} – {lbl}")
        self._fmt_axes(ax, dates)
    ...
    def _draw_chart3(self):
        ...
        if show_total:
            ln, = ax.plot(x, [all_total[d] for d in dates],
                          color="black", ls="-", marker="D", markersize=5, lw=2.5,
    ...
    def _draw_chart4(self):
        ...
        for fab in active:
            for k, (lbl, ls, mk) in SEVK_METRICS.items():
                ...
                ax.plot(x, self._s_series(k, fab, dates), color=FAB_COLORS[fab],
                        ls=ls, marker=mk, markersize=4, lw=1.6,
                        label=f"{fab} – {lbl}")

        if self._c4_total_var.get():
            # Tüm fabrikaların toplam sevki (toplam_sevk sütununun toplamı)
            total = [
    ...
    def _draw_chart5(self):
        ...
        self._fmt_axes(ax, all_dates)
        ax.set_title(f"Üretim ({u_lbl}) vs Sevkiyat ({s_lbl})",
                     fontsize=13, fontweight="bold", pad=10)
        ax.set_ylabel("Miktar (m²)")
        # Stil notu: düz=üretim, kesikli=sevk
